...
/**
 * Esercizio: Calcolatrice da Terminale con Menù e Cronologia
 *
 * Consegna: Implementare una calcolatrice a linea di comando con menù
 * interattivo. L'utente sceglie l'operazione (somma, sottrazione,
 * moltiplicazione, divisione), inserisce i due operandi e vede il risultato.
 * Il programma memorizza gli ultimi 5 risultati e permette di visualizzarli.
 * L'utente può riutilizzare gli stessi numeri o inserirne di nuovi.
 *
 * Requisiti: gestione divisione per zero, cronologia circolare degli ultimi 5
 * risultati, validazione completa dell'input.
 *
 * Concetti applicati: classi separate (Calc), array circolari, switch-case,
 * ciclo while(true) con break, gestione eccezioni, interazione utente.
 */
class MenuCalc {
  public static void main(String args[]) throws IOException {
    ...
    float[] risultati = new float[5];   // cronologia circolare
    int nextResult = 0;
    int countResults = 0;
    ...
    while (true) {
      ...
      // Opzione 6: cambia i numeri salvati
      if (opzione == 6) {
        try {
      ...
      // Opzione 5: mostra la cronologia degli ultimi risultati
      if (opzione == 5) {
        if (countResults == 0) {
      ...
      // Operazioni 1–4: richiede i numeri se non già presenti
      if (opzione >= 1 && opzione <= 4) {
        if (!haveNumbers) {
          ...
          // Chiede se riutilizzare i numeri già inseriti
        ...
        // Salva nella cronologia circolare solo se l'operazione è valida
        if (validOperation) {
